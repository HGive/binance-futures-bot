...
def create_limit_order(exchange, symbol, side, amount, price ):
    try:
...
def create_take_profit_order(exchange, symbol, side, amount, price, stop_price ):
    try:
        order = exchange.create_order(
            symbol=symbol, type="TAKE_PROFIT", side=side, amount=amount,
            price=price, params={'stopPrice': stop_price} )
        
        # 주문 상태 확인
        status = order['status']
        if status in ['open', 'closed', 'partially_filled']:
            logging.info(f"Take profit order created successfully. Status: {status}")
            return order
        else:
            logging.warning(f"Unexpected order status: {status}. Retrying...")
            time.sleep(interval)
