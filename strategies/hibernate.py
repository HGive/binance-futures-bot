"""
HIBERNATE (동면) v1.0 — 롱 전용 현물 전략

  겨울잠 자는 코인을 미리 사놓고, 깨어날 때 판다.

  슈팅 이력은 있는데(= 깨어날 능력은 증명됨) 한동안 조용하고(= 지금은 동면 중)
  가격도 바닥권인 종목을 소량 담아두고 기다린다.

손매매로 실제 수익을 내던 방식을 그대로 옮긴 것.
...
검증 성적 (워크포워드 8년 24구간, 현물 540종 상폐 포함):
  총 +123.8% (연 14.6%) / MDD -23.6% / PF 2.09 / 승률 60.5% / 119거래
  탐색에 안 쓴 2024년 이후 구간: +31.3%

민감도 (backtest/hib_tune.py, 워크포워드 24구간 / 2024년 이후 10구간)
  상수 12개를 55가지로 흔들어봤다. **양쪽이 다 좋아진 변경은 없다.**
  대표적으로 (전체 / 2024년 이후):
    SPIKE_MIN_PCT 0.35 → 0.25/0.30/0.40/0.50   -97/-83/-72/-86%p  /  -33/-39/-37/-37%p
    MIN_SPIKES 2 → 1 / 3                        -93 / -81%p       /  -46 / -39%p
    STOP_PCT -0.20 → -0.12/-0.15/-0.25/-0.30    -23/-44/-60/-64%p /  -23/-34/-32/-29%p
    MIN_TGT_RATIO 0.25 → 0.20 / 0.30            -84 / -79%p       /  -44 / -15%p
    MAX_CONCURRENT 8 → 6 / 10 / 12              -42/-25/-64%p     /  -14/-16/-48%p
  TICKER_MARGIN_PCT 0.10 → 0.12 만 양쪽이 오르는데(+33 / +5%p) MDD 도 -23.6% → -27.6%
  로 같이 커진다. 엣지가 아니라 위험/수익 다이얼이다.

  **읽는 법**: 상수들이 이 데이터에서 뾰족한 봉우리 위에 있다는 뜻이다.
  값을 여기서 고른 이상 당연한 결과지만, 실전 성적이 백테스트보다 나쁠 가능성을
  높게 봐야 한다는 신호이기도 하다.

변경 이력
  v1.0  2026-08-24  워크포워드 통과. 2단 익절(적응형 목표 70% + 2배 러너 30%),
                    자리 밴드 0.25~0.60, 고정 손절 -20%, 물타기 없음, BTC MA100 레짐.
...
MAX_HOLD_DAYS = 0            # 0 이면 시간 제한 없음. 오래 묶인 자리를 비울지 여부
...
# =====================================================================
#  아래 세 상수(TP_MIN, TP_MAX, MAX_TGT_RATIO)는 8년 데이터에서 **한 번도 걸리지 않았다.**
#  값을 0.05~0.15 / 0.5~1.0 / 0.45~0.9 로 바꿔도 결과가 소수점까지 동일하다.
#  즉 성적에 맞춰 고른 값이 아니라 안전 난간이다. 실제로 조정된 상수는 그만큼 적다.
#  (측정: backtest/hib_tune.py)
